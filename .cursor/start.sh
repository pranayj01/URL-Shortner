...
# Starts PostgreSQL and Redis, provisions the database, and applies Prisma
# migrations. Safe to run repeatedly (idempotent). System packages are
# installed by .cursor/install.sh, not here.
set -euo pipefail
...
# 2. Provision the role password and application database (idempotent).
